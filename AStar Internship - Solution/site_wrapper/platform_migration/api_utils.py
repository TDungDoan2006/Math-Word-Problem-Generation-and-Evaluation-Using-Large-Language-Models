...
class Azure():
    ...
    def get_GPT_txt_response(self, sys_prompt, user_prompt, gpt_model=GPT_MODEL, openai_client = None):
        start_t = time.time()
    ...
    def get_GPT_struct_response(self, sys_prompt, user_prompt, struct_type, gpt_model=GPT_MODEL, openai_client = None):
        start_t = time.time()
        if openai_client is None:
            openai_client = self.create_openai_client()
        try:
    ...
    def create_client(self,endpoint=EndPoint, api_version=API_Version):
    ...
    def get_LLM_response(self, model_name, sys_prompt, user_prompt, client = None):
        start_t = time.time()
        if client is None:
        ...
        try:
            ...
            self.llm_response = {'response': response.choices[0].message.content, 'status': 0}
        except Exception as e:
            self.llm_response = {'response': '', 'status': -1}
            print(f">>> Error processing submission: {str(e)}")
        response_time = time.time()-start_t
        self.llm_response['response_time'] = response_time
...
class Aws():
    ...
    def get_LLM_response(self, model_id, sys_prompt = '', user_prompt = '', config = {}, client = None):
